...
function addBubbleEvent(bubble){
   bubble.addEventListener('click', () => {
    const options = {
        ...
        fetch('https://api.elevenlabs.io/v1/text-to-speech/jB108zg64sTcu1kCbN9L', options)
        .then(async (response) => {
    ...
    if (userValue != "") {
        ...
        if (contentValue == 1) {
            if (checkImgPromt(userValue)) {
                ...
                textGen({"inputs": userValue, "parameters": {"return-full-text": false}})
                    .then(async (response) => {
                        let aiContentValue = await contentFilterText(response[0].gererated_text)
                        if (aiContentValue == 1) {
                            frame.appendChild(userInput)
                            frame.appendChild(aiOutput)
                            userInput.innerHTML = userValue
